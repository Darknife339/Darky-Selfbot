...
class Embed:
    def __init__(self, title="", description="", colour="#8B5CF6", color=""):
        self.title = title
        ...
        try:
            ...
            self.description_font = ImageFont.truetype("arial.ttf", 54)
            self.description_font_bold = ImageFont.truetype("arial.ttf", 54)
            self.footer_font = ImageFont.truetype("arial.ttf", 54)
        except:
            self.title_font = ImageFont.load_default()
            self.description_font = ImageFont.load_default()
            self.description_font_bold = ImageFont.load_default()
        ...
        self.waves = Image.new("RGBA", (200, 200), (0, 0, 0, 0))

    def set_thumbnail(self, url=""):
        self.thumbnail = url
        if url:
            try:
                self.thumbnail_resp = requests.get(url, timeout=10)
            except:
                self.thumbnail_resp = None
    ...
    def setup_dimensions(self):
        ...
        if len(self.fields) > 0:
            for field in self.fields:
                ...
                for line in field.value.splitlines():
                    wrap = get_wrapped_text(
                        line, self.description_font, self.wrap_width
                    )
                    for _ in wrap:
                        self.height += 60
        ...
        if self.footer != "":
            self.height += 100
        if self.thumbnail != "" and self.height < 400:
            self.height = 400

    def draw_title(self, template, draw):
        if self.title != "":
            if self.thumbnail != "":
                title_bg_img = Image.new(
                    "RGBA", (self.width - 300 - 40 - 40 - 40, 135), (0, 0, 0, 0)
                )
                ImageDraw.Draw(title_bg_img).rounded_rectangle(
                    [(0, 0), (title_bg_img.width, title_bg_img.height)],
                    20,
                    fill=(0, 0, 0, 50),
                )
                ...
                title_bg_img = Image.new(
                    "RGBA", (self.width - 45 - 40, 135), (0, 0, 0, 0)
                )
                ImageDraw.Draw(title_bg_img).rounded_rectangle(
                    [(0, 0), (title_bg_img.width, title_bg_img.height)],
                    20,
    ...
    def draw_thumbnail(self, template, draw):
        if (
            self.thumbnail != ""
            and self.thumbnail_resp
            and self.thumbnail_resp.status_code == 200
        ):
            try:
    ...
    def draw_description(self, template, draw):
        if self.description != "":
            ...
            for desc_line in self.description.splitlines():
                wrap = get_wrapped_text(
                    desc_line, self.description_font, self.wrap_width
                )
                ...
                for line in wrap:
                    ...
                    for word in new_line:
                        if word.startswith("**") and word.endswith("**"):
                            ...
                            x_offset += self.description_font_bold.getlength(
                                word.replace("**", "")
                            )
                        else:
                            draw.text(
        ...
        if len(self.fields) > 0:
            ...
            for field in self.fields:
                ...
                for field_line in field.value.splitlines():
                    wrap = get_wrapped_text(
                        field_line, self.description_font, self.wrap_width
                    )
    ...
    def draw_background(self, template, draw):
        rgb = hex_to_rgb(self.colour)
        draw.rectangle([(0, 0), (10, self.height)], fill=rgb)
    ...
    def save(self):
        extension = ".png"
        path = f"embed-{random.randint(1000, 9999)}{extension}"
